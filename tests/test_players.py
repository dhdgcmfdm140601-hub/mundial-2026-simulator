...
def test_create_player_duplicate_in_same_team(client: TestClient):
    team = _create_team(client)
    client.post("/players/", json={"name": "Lionel Messi", "position": "FW", "team_id": team["id"]})
    res = client.post("/players/", json={"name": "Lionel Messi", "position": "MF", "team_id": team["id"]})
    assert res.status_code == 409
    assert "player" in res.json()["detail"].lower()


def test_create_player_same_name_different_team(client: TestClient):
    t1 = _create_team(client, "Argentina", "ARG")
    t2 = _create_team(client, "Brasil", "BRA")
    client.post("/players/", json={"name": "Lionel Messi", "position": "FW", "team_id": t1["id"]})
    res = client.post("/players/", json={"name": "Lionel Messi", "position": "FW", "team_id": t2["id"]})
    assert res.status_code == 201


def test_update_player_duplicate_name_in_team(client: TestClient):
    team = _create_team(client)
    client.post("/players/", json={"name": "Messi", "position": "FW", "team_id": team["id"]})
    res = client.post("/players/", json={"name": "Di Maria", "position": "MF", "team_id": team["id"]})
    pid = res.json()["id"]
    res = client.put(f"/players/{pid}", json={"name": "Messi"})
    assert res.status_code == 409
    assert "player" in res.json()["detail"].lower()


def test_update_player_move_to_team_with_duplicate(client: TestClient):
    t1 = _create_team(client, "Argentina", "ARG")
    t2 = _create_team(client, "Brasil", "BRA")
    client.post("/players/", json={"name": "Messi", "position": "FW", "team_id": t1["id"]})
    res = client.post("/players/", json={"name": "Messi", "position": "FW", "team_id": t2["id"]})
    pid = res.json()["id"]
    res = client.put(f"/players/{pid}", json={"team_id": t1["id"]})
    assert res.status_code == 409
    assert "player" in res.json()["detail"].lower()


def test_create_player_empty_name(client: TestClient):
    team = _create_team(client)
    res = client.post("/players/", json={"name": "", "position": "FW", "team_id": team["id"]})
    assert res.status_code == 400


def test_max_players_per_team(client: TestClient):
    team = _create_team(client)
    for i in range(23):
        res = client.post("/players/", json={"name": f"Player {i}", "position": "FW", "team_id": team["id"]})
        assert res.status_code == 201
    res = client.post("/players/", json={"name": "Extra Player", "position": "FW", "team_id": team["id"]})
    assert res.status_code == 400
    assert "23" in res.json()["detail"]